...
class TaskService {
  ...
  async add(card_id: number, title: string, description: string | null, start: string | null, deadline: string | null, percent: number) {
    try {
      return (await pool.query<Task>(`
          insert into task (card_id, title, ${description ? 'description ,' : ''} 
           ${start ? 'start ,' : ''} ${deadline ? 'deadline ,' : ''} percent, completed)

          VALUES (${card_id}, '${title}',
                  ${description ? `'${description}' ,` : ''} ${start ? `'${start}' ,` : ''}
                      ${deadline ? `'${deadline}' ,` : ''} ${percent},
                  ${false})
          returning *`)).rows[0]
    } catch (e) {
      console.log(e)
